...
int main()
{
	...
	if (conf.auto_gpbip==true){if (gpb_ip(sock,&ip,&port)==0){std::cout<<"your public ip and port: "<<ip<<' '<<port<<'\n';}}
	while (1)
	{
		std::string cmd;
		...
		if (cmd=="conn")
		{
			...
			std::string i_ip;
			while (i_ip.empty()){getline(std::cin,i_ip);}
			std::cout<<"Enter interlocutor port: ";
			std::string i_p;
			while (i_p.empty()){getline(std::cin,i_p);}
